...
class PhonemeTransform:
    ...
    def getunknowns(self,words):
        unknown_words = set([word for word in words if word.lower() not in self.phodict.keys()])
    ...
    def _getphoneme(self,word):
        phoneme = ''.join(self.phodict[word][0]) # always use first pronuciations at index 0
        return phoneme
    ...
    def invertdictionary(self):
        ...
        for word in self.phodict:
            ...
            phoneme = ''.join(self.phodict[word][0]) # always use first phoneme for a word
...
class PhonemeReverse(PhonemeTransform):
    ...
    def getenglish(self, text, runs):
        ...
        for i in range(runs):
            ...
            if i>0:
                baseline ='[CLS] '+ TextFormat.arraytotext(self,englishlist) + ' [SEP]'
                englishlist = []
            for index, word in enumerate(wordsarray):
                if len(word)>1 :
                    ...
                    if type(word[1]) != int :
                        ...
                        try:
                            best_word = pd.merge(predicted_words,wordlist, on ='Word')\
                            .sort_values(0, ascending= False,ignore_index=True).loc[0,'Word']
                        except:
                            best_word=word[0]
                    ...
                    # So instead use the highest ranked word with the same syllable count
                        ...
                        predicted_words.sort_values(0, ascending= False, inplace = True,ignore_index=True)
                        ...
                        for pred_word in predicted_words['Word']:
                            ...
                            try:
                                pred_syllable = super(PhonemeReverse, self).syllablecount(self._getphoneme(pred_word))
